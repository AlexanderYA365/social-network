    ...
    <div class="row">
        <div class="col-sm">
                    ...
                    <div class="btn-toolbar">
                        <div>
                            <form action="account-message" method="POST">
                                <button type="submit" class="btn btn-success px-4 py-1" name="selectUser"
                                        value=${friend.id}>
                                    Личное сообщение
                                </button>
                            </form>
                        ...
                        <div>
                            <c:if test="${friendFlag == 1}">
                                <form method="post">
                                    <button type="submit" class="btn btn-success px-4 py-1" name="add">
                                        Заявка в друзья
                                    </button>
                                </form>
                            </c:if>
                            <c:if test="${friendFlag == 2}">
                                <form modelAttribute="friend" method="post">
                                    <button type="submit" class="btn btn-success px-4 py-1" name="add-account">
                                        Заявка в друзья
                                    </button>
                                </form>
                            </c:if>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    Имя: ${friend.accountDetails.name}
                    <br>Фамилия: ${friend.accountDetails.surname}
                    <br>Отчество: ${friend.accountDetails.lastName}
                    <br>Дата рождения: ${friend.accountDetails.date}
                    <br>icq: ${friend.accountDetails.icq}
                    <c:forEach var="phone" items="${friend.phones}">
                    ...
                    <br>Домашний адрес: ${friend.accountDetails.addressHome}
                    <br>Рабочий адрес: ${friend.accountDetails.addressJob}
                    <br>email: ${friend.accountDetails.email}
                    <br>Обо мне: ${friend.accountDetails.aboutMe}
                </div>
            </div>
        </div>
        <div class="col-sm">
            <div class="card">
                ...
                <c:if test="${friendFlag == 1}">
                    Вам отказано писать сообщения на стену пользователя
                </c:if>
                <c:if test="${friendFlag == 2}">
                    <p class="text-center">
                        Пользователь отказал вам в заявке на добавление в друзья,
                        Вам отказано писать сообщения на стену пользователя
                    </p>
                </c:if>
                <c:if test="${friendFlag == 0}">
                    <div class="card-body">
                        <form method="post" action="${pageContext.request.contextPath}/friend-add-wall-message">
...
<script>
    ...
    async function getData(page) {
        var requests = [];
        requests.push('/get-size-friend-wall-messages/');
        requests.push('/get-friend-wall-messages/');
        requests.push('/get-friend-senders-avatar/');
        requests.push('/get-friend-sender-account/');
